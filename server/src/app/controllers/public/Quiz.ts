...
export default {
  async getQuizzesPublic(req: Request, res: Response) {
    try {
      ...
      const usersCreator = await Promise.all(
      ...
      const quizzesWitchCreator: IQuizSentToCustomer[] = quizzes.map((quiz) => {
        const creatorInfos = usersCreator.find(
          (user) => user?._id.toString() === quiz.creatorId
        )
        return {
  ...
  async checkAnswers(req: Request, res: Response) {
    ...
    try {
      ...
      const currentMatch = await recordMatchData({
        authToken:
          req.cookies[process.env?.NAME_TOKEN_AUTORIZATION || ''] || '',
      ...
      const achievement: IAchievementSentByServer | null =
        await checkAndSaveUserGame({ currentMatch, quiz })
