...
import { userCreatedQuizzes } from '../../utils/quiz/refactoringQuizes'
...
export default {
  async creatingQuiz(req: Request, res: Response) {
    const quiz: IQuizClient = req.body
  ...
  async getQuizesCreateBy(req: Request, res: Response) {
    try {
      const token = req.cookies[process.env.NAME_TOKEN_AUTORIZATION || '']
      const resCreatorId: IResponseCreatorId = returnCreatorId(token)
      ...
      res.status(200).send({
        ...
        data: { quizzes: userCreatedQuizzes(quizzes) },
      })
    } catch (error) {
      console.log(`Ocorreu um erro ao criar o quiz. error: ${error}`)
      res.status(500).send({
